...
public interface IdRemoteMap<I, V> extends fpc.tools.state.RemoteMapBase<I,V> {
    ...
    static <I, V> @NonNull IdRemoteMap<I, V> empty(@NonNull Function1<? super V, ? extends I> idGetter) {
        return FPCIdRemoteMap.empty(idGetter);
    }

    static <I, V extends Identified<I>> @NonNull IdRemoteMap<I, V> empty() {
        return FPCIdRemoteMap.empty(Identified::getIdentification);
    }

    IdRemoteMap<I,V> remove(@NonNull I key);
     ...
     */
    @NonNull
    IdRemoteMap<I,V> removeIfKeyMatches(@NonNull Predicate<? super I> predicate);

    /**
     * @param value the value to put
     * @return a new MapState with the same values of this plus the provided entry.
     */
    IdRemoteMap<I, V> put(@NonNull V value);
     ...
     */
    IdRemoteMap<I, V> replace(@NonNull V value);
     ...
     *
     */
    IdRemoteMap<I, V> updateValue(@NonNull V value, @NonNull Comparator<? super V> isNewer);

    /**
     * Same as {@link #updateValue(Object, Comparator)} but for several values at once
     */
    IdRemoteMap<I, V> updateValues(@NonNull Collection<V> newValues,
     ...
     */
    <T> IdRemoteMap<I, V> updateItems(@NonNull Collection<T> items,
